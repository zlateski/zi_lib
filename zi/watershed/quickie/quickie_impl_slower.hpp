                    ...
                    std::vector< tuple< SegmentIdType, SegmentIdType, ValueType > >& dendr )
{
    ...
    zi::sort( dend_edges.begin(), dend_edges.end(),
              detail::affinity_edge_sorter<value_type,index>(conn) );
    ...
    for ( size_type i = 0; i < dend_edges.size(); ++i )
    {
        // extract src/targed
        const id_type n1 = dend_edges[i] % size;
        const id_type n2 = n1 + rindex[dend_edges[i]/size];

        const id_type v1 = sets[seg[n1]];
        const id_type v2 = sets[seg[n2]];

        ZI_ASSERT(v1&&v2);

        if ( conn[dend_edges[i]] >= high )
        {
            if ( v1 != v2 )
            {
        ...
        else if ( conn[dend_edges[i]] >= dust_low )
        {
            if ( v1 != v2 )
            {
    ...
    for ( size_type i = 0; i < dend_edges.size(); ++i )
    {
        ...
        const id_type a1 = remaps[seg[n1]];
        const id_type a2 = remaps[seg[n2]];
        ...
        {
            sets.join( v1, v2 );
            dendr.push_back(edge_type( a1, a2, conn[dend_edges[i]] ));
        }
    }
