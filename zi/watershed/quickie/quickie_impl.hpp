...
struct heap_entry
{
    static const T not_in_heap = ~static_cast<T>(0);
    ...
    heap_entry():
        heap_pos(~static_cast<T>(0)), aff_value(), seg_index(), seg_parent()
    { }

    typedef heap_entry* pointer;
};

template <typename V, typename T>
void push_heap( typename heap_entry<V,T>::pointer *heap, std::size_t n )
{
    typename heap_entry<V,T>::pointer p = heap[n];
    std::size_t parent = (n-1)/2;
    while ( n > 0 && (heap[n]->aff_value > heap[parent]->aff_value) )
    {
        heap[n] = heap[parent];
        heap[n]->heap_pos = n;
        n = parent;
...
template <typename V, typename T>
void pop_heap( typename heap_entry<V,T>::pointer *heap, std::size_t n )
{
    if ( n == 0 )
    {
        return;
    ...
    heap[n] = heap[0];
    heap[n]->heap_pos = heap_entry<V,T>::not_in_heap;

    std::size_t idx   = 0;
    std::size_t child = 1;
    while ( child < n )
    {
        if ( child+1 < n && heap[child+1]->aff_value>heap[child]->aff_value )
        {
            ++child;
        }

        if ( heap[idx]->aff_value > heap[child] )
        {
            child = n;
        }
        else
