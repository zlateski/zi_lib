...
class quickie_impl: non_copyable
{
private:

    typedef IdType id_type;
    typedef quickie_impl< AffinityType, IdType, SizeType > type;
...
private:
    ...
    inline const std::pair< const id_type&, const id_type& >
    min_max_pair( const id_type &a, const id_type &b ) const
    {
        return a < b
            ? std::pair< const id_type&, const id_type& >( a, b )
            : std::pair< const id_type&, const id_type& >( b, a );
    }
                    ...
                    for ( std::size_t x = 0; x < chunk_.x(); ++x, ++j, ++i )
                    {
                        ...
                        if ( conn[ i ] >= dend_min_ )
                        {
                            if ( ids[ j ] &&
                                 ...
                                 ids[ j ] != ids[ j + nhood[ d ] ] )
                            {
                                IdPair x = min_max_pair( ids[ j ], ids[ j + nhood[ d ] ] );
...
std::size_t quickie( affinity_matrix< AffinityType >& conn,
                     ...
                     bool verbose = true )
{
    typedef quickie_impl< AffinityType, IdType, SizeType > QuickieType;
